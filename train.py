...
import argparse
import random
from utils import apply_content_loss, adjust_scales2image, generate_noise2, calc_gradient_penalty, get_content_model_and_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quantize_image', type=int, help='number of colors for spacial image (default is uncompressed, max is 256)', default=0)
    parser.add_argument('--content', type=int, help='conv layer to apply content loss on', default=4)
    ...
    opt.content_scale = opt.switch_res * 2
    ...
    while scale_num < opt.stop_scale + 1:
        ...
        if (scale_num < opt.content_scale):
            print("Content layer")
        else:
            print("No Content layer")
        ...
        # finished training at this scale
        if (scale_num < opt.content_scale) and (opt.content > 0):
            # content loss
            
            content_a = apply_content_loss(opt, G_a, prev_b.detach().float())
            content_b = apply_content_loss(opt, G_b, prev_a.detach().float())
            
            vutils.save_image(content_a.clone(), osp.join(opt.out, "content_a_{}.png".format(scale_num)), normalize=True)
            vutils.save_image(content_b.clone(), osp.join(opt.out, "content_b_{}.png".format(scale_num)), normalize=True)
        ...
        if scale_num == opt.stop_scale:
            ...
            if opt.quant:
                ...
                vutils.save_image(fake_a.clone(), osp.join(opt.out, "pre_quant_a.png"), normalize=True)
                vutils.save_image(fake_b.clone(), osp.join(opt.out, "pre_quant_b.png"), normalize=True)
                ...
                mix_quant_a = G_a(quant_b)
                mix_quant_b = G_b(quant_a)
            ...
            if opt.quant:
                vutils.save_image(mix_quant_a.clone(), osp.join(opt.out, "final_quant_b2a_" + str(epoch) + ".png"),
                              normalize=True)
                vutils.save_image(mix_quant_b.clone(), osp.join(opt.out, "final_quant_a2b_" + str(epoch) + ".png"),
                              normalize=True)
